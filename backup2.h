...
class backup2 :public Strategy {
	...
	double getWeight(int botId, int wbId1, int wbId2, int goodId) {
		...
		double _disBot2Wb1 = calDistSq(botArr[botId].x, botArr[botId].y, wbArr[wbId1].x, wbArr[wbId1].y);
	...
	// 在当前地图中，根据getWeight2()计算的权重，查找botID最适合拿来sell的工作台wbArr[wb2Id]
...
void backup2::afterReadFrame(int frameId) {
...
void backup2::competeTar(int botId) {
	...
	if (botTarId[botId][0] == 1) {		// 如果此时机器人botId当前奔向的是buy目标
		...
		if (robFlag == false) {		// 如果抢别人的目标不成功，则试图找没有机器人锁定的buy工作台，且更新后能小赚
			...
			if (wbId1 != -1 && wbId2 != -2) {
				...
				//hwcout << "newBuyTar " << wbId1 << hwendl;
				//hwcout << "oldSellTar " << botIdid2 << hwendl;
				//hwcout << "newSellTar " << wbId2 << hwendl;
				//hwcout << "botIdLastWeight " << botIdLastWeight << hwendl;
				//hwcout << "botIdCurWeight " << curWeight << hwendl;
